...
#' Download all files to multiple zip files (currently just .csv data files, but options could be added to allow for download of all types of files) in a Year component.
#'
#' @param id An OSF id to a year component. If no id is specified, will use the current year id.
#' 
#' @param path Like "path/to/folder" The path to which the file should be downloaded EXCLUDING A FILE NAME. File names will be autogenerated based on project names.
#'
#' @export download_all_files

download_all_files <- function(id = NULL, path) {
  if (is.null(id)) {
    # Ag division Node
    id <- "judwb"
  ...
  for (j in seq_along(res$data)) {
    ...
    download_files(proj_id, component = TRUE, folders = TRUE, path = to_path)
  }
...
#'
#' @param year The current year component or year component to which file moving is desired. If NULL, defaults to current year.
#' 
...
move_to_projects <- function(id = "judwb", year = NULL, data_comp = "Raw_Data", from = 1, to = NULL) {
  if (is.null(year)) {
